...
export function QuizSection() {
  ...
  const inputClass =
    "w-full bg-[var(--navy-deep)] border border-[var(--divider)] focus:border-[var(--blue-brand)] text-[var(--text-primary)] placeholder:text-[var(--text-secondary)] px-5 py-4 text-base outline-none transition-colors duration-200"

  return (
    <section className="py-24 bg-[var(--navy-card)] border-y border-[var(--divider)]" aria-labelledby="quiz-heading">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start">
            ...
            <div className="flex flex-col gap-3">
              {[
                ...
                <div key={item} className="flex items-center gap-3">
                  <CheckCircle2 size={16} className="text-[var(--blue-brand)] shrink-0" aria-hidden="true" />
          ...
          {/* Right: quiz card */}
          <div className="bg-[var(--navy-deep)] border border-[var(--divider)]">
            {/* Progress */}
            <div className="flex gap-px">
              {Array.from({ length: totalSteps }).map((_, i) => (
                <div
                  ...
                  style={{ background: i <= step ? "var(--blue-brand)" : "var(--divider)" }}
                  aria-hidden="true"
                />
              ))}
            </div>
            ...
            <div className="p-8 lg:p-10">
              {submitted ? (
                <div className="flex flex-col items-center text-center py-8 gap-5">
                  <CheckCircle2 size={56} className="text-[var(--blue-brand)]" aria-hidden="true" />
                  <h3 className="text-[var(--text-primary)] font-black text-2xl">Заявка принята!</h3>
                  <p className="text-[var(--text-secondary)] text-base leading-relaxed max-w-sm">
                    Технический директор свяжется с вами в течение 30 минут и даст предварительную оценку.
                  </p>
                </div>
              ) : isFormStep ? (
                <>
                  <div className="flex items-center gap-3 mb-8">
                    ...
                    <p className="text-[var(--text-secondary)] text-xs uppercase tracking-widest font-semibold">
                      Шаг {step + 1} из {totalSteps} — Ваши контакты
                    </p>
                  </div>
                  <h3 className="text-[var(--text-primary)] font-bold text-xl mb-6">
                  ...
                  <form onSubmit={handleSubmit} noValidate>
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="mt-6 w-full bg-[var(--blue-brand)] hover:bg-[var(--blue-hover)] disabled:opacity-60 text-white font-bold text-sm px-6 py-4 uppercase tracking-wider transition-colors duration-200 flex items-center justify-center gap-3 cursor-pointer disabled:cursor-not-allowed"
                    >
                      {loading ? (
                        <>
                  ...
                  <div className="flex flex-col gap-2">
                    {currentStep.options.map((opt) => (
                      <button
                        ...
                        className="w-full text-left px-5 py-4 border border-[var(--divider)] hover:border-[var(--blue-brand)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] font-medium text-sm transition-colors duration-150 cursor-pointer flex items-center justify-between group"
                        style={{
                          background: answers[currentStep.id] === opt.value ? "var(--blue-brand)" : "var(--navy-card)",
                          color: answers[currentStep.id] === opt.value ? "white" : undefined,
                          borderColor: answers[currentStep.id] === opt.value ? "var(--blue-brand)" : undefined,
                        }}
                      >
                        {opt.label}
